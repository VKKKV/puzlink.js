...
import { ConeLogProbs } from "../src/metrics/coneLogProb.js";
...
const missingFeatures = new Set(Object.keys(ConeLogProbs.metricLogProbs));

ConeLogProbs.wrapCompute = (name, fn, existing) => {
  process.stdout.write(`  ${name}...`);
  const start = Date.now();
  const result = fn();
  const duration = Date.now() - start;
  const tag = existing === undefined ? "(new)" : "(diff)";
...
for (const line of ConeLogProbs.dump()) {
  newFile.push(line);
}
